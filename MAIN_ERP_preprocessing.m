...
for subjectNo = 1:length(SETTINGS.filenames)
    % Load EEG data
    EEG = loadEEGfile(SETTINGS.pathname, ...
                      SETTINGS.filenames{subjectNo}, ...
...
% rereference =============================================================
        case 'rereference'
            EEG = pop_reref(EEG, ...
                            find(ismember({EEG.chanlocs.labels}, ...
                                           SETTINGS.newReference)), ...
                            'keepref', 'on');
...
% lowPassFilter ===========================================================       
        case 'lowPassFilter'
            ...
            EEG = addHistory(EEG, ...
                             "lowPassFilter", ...
                             sprintf("Cutoff: %g ,Design: %s, filter_type: %s, order: %g", ...
                                      SETTINGS.lowPassFreq, "butter", "lowpass", 6));
            recursivelySaveFile(EEG, ...
                                SETTINGS.pathname, ...
                                'LowPassFilter', ...
                                SETTINGS.filenames{subjectNo});
...
% highPassFilter ==========================================================
        case 'highPassFilter'
            ...
            EEG = addHistory(EEG, ...
                             "highPassFilter", ...
                             sprintf("Cutoff: %g ,Design: %s, filter_type: %s, order: %g", ...
                                     SETTINGS.highPassFreq, "butter", "highpass", 6));
            recursivelySaveFile(EEG, ...
                                SETTINGS.pathname, ...
                                'HighPassFilter', ...
                                SETTINGS.filenames{subjectNo});
...
% notch ===================================================================        
        case 'notch'
            ...
            EEG = addHistory(EEG, ...
                             "notchFilter", ...
                             sprintf("Cutoff: %g , Design: notch, filter_type: PMnotch, order: %g", ...
                                    SETTINGS.notchFreq, 180));
            recursivelySaveFile(EEG, ...
                                SETTINGS.pathname, ...
                                'NotchFilter', ...
                                SETTINGS.filenames{subjectNo});
...
% epoch ===================================================================            
        case 'epoch'
            ...
            EEG_Stim1 = pop_epochbin(EEG_Stim1 , ...
                    ...
                    'warning', 'off');    
            EEG_Stim1 = addHistory(EEG_Stim1, ...
                                   "epoch", ...
                                   sprintf("tmin: %g , tmax: %g, eventlistFile: %s", ...
                                   -abs(SETTINGS.prestimDur), SETTINGS.poststimDur, eventlistFile));
            recursivelySaveFile(EEG_Stim1, ...
                                SETTINGS.pathname, ...
                                toSaveFolder, ...
                                SETTINGS.filenames{subjectNo}, ...
...
%epochSelectively =========================================================            
        case 'epochSelectively'
            ...
            EEG_STD_PRE  = pop_editeventlist(EEG, ...
                                            'AlphanumericCleaning', 'on', ...
                                            'BoundaryNumeric', { -99}, ...
                                            'BoundaryString', {'boundary'}, ...
                                            'List',fullfile(curr_path, ...
                                                            'Eventlists/eventlist_STD_PRE.txt'), ...
                                            'SendEL2', 'EEG', ...
                                            'UpdateEEG', 'on', ...
                                            'Warning', 'off' ); 
                EEG_STD_PRE = pop_epochbin(EEG_STD_PRE, ...
                                          [-abs(SETTINGS.prestimDur) SETTINGS.poststimDur],'pre', ...
                                          'warning', 'off');    
                recursivelySaveFile(EEG_STD_PRE, ...
                                    SETTINGS.pathname, ...
                                    'Standard_PRE_Epoched', ...
                                    SETTINGS.filenames{subjectNo}, ...
                                    'FileNameSuffix', 'STD_PRE');
                clear EEG_STD_PRE
                                        
            EEG_DEVIANT  = pop_editeventlist(EEG, ...
                                            'AlphanumericCleaning', 'on', ...
                                            'BoundaryNumeric', {-99}, ...
                                            'BoundaryString', {'boundary'}, ...
                                            'List', fullfile(curr_path, ...
                                                            'Eventlists/eventlist_DEVIANT.txt'), ...
                                            'SendEL2', 'EEG', ...
                                            'UpdateEEG', 'on', ...
                                            'Warning', 'off' ); 
                EEG_DEVIANT = pop_epochbin(EEG_DEVIANT, ...
                                           [-abs(SETTINGS.prestimDur) SETTINGS.poststimDur],'pre', ...
                                           'warning', 'off');    
                recursivelySaveFile(EEG_DEVIANT, ...
                                    SETTINGS.pathname, 'DEVIANT_Epoched', ...
                                    SETTINGS.filenames{subjectNo}, ...
                                    'FileNameSuffix', 'DEVIANT');
                clear EEG_DEVIANT
            
            EEG_STD_POST  = pop_editeventlist(EEG, ...
                                              'AlphanumericCleaning', 'on', ...
                                              'BoundaryNumeric', {-99}, ...
                                              'BoundaryString', {'boundary'}, ...
                                              'List',fullfile(curr_path, ...
                                                             'Eventlists/eventlist_STD_POST.txt'), ...
                                              'SendEL2', 'EEG', ...
                                              'UpdateEEG', 'on', ...
                                              'Warning', 'off' ); 
                EEG_STD_POST = pop_epochbin(EEG_STD_POST, ...
                                            [-abs(SETTINGS.prestimDur) SETTINGS.poststimDur],'pre', ...
                                            'warning', 'off');    
                recursivelySaveFile(EEG_STD_POST, ...
                                    SETTINGS.pathname, ...
                                    'Standard_POST_Epoched', ...
                                    SETTINGS.filenames{subjectNo}, ...
                                    'FileNameSuffix', 'STD_POST');
                clear EEG_STD_POST
...
%artifactsRejection =======================================================
        case 'artifactsRejection'
           ...
           if SETTINGS.usePrevRejEpochs
               ...
               fprintf(">>> Rejecting trials with amplitude exceeding [%g; %g] uV\n", SETTINGS.lowAmplitudeThreshold, SETTINGS.highAmplitudeThreshold)
               ...
               fprintf(">>> Rejecting trials with sample-to sample amplitude over %g uV\n", SETTINGS.sampleToSample)
               EEG  = pop_artdiff(EEG, ...
                                  'Channel',1:EEG.nbchan, ...
                                  'Flag',1, ...
                                  'Threshold', SETTINGS.sampleToSample, ...
                                  'Twindow', [SETTINGS.minTimeWindowArtifacts ...
                                              SETTINGS.maxTimeWindowArtifacts]); 
               
               %exclude trials with too low amplitude )flat electrodes)
               fprintf(">>> Rejecting trials with amplitude between [%g; %g] uV lasting for min %g ms.\n", -1*SETTINGS.noActivity, SETTINGS.noActivity, SETTINGS.noActivityDuration)
               EEG  = pop_artflatline(EEG, ...
                                      'Channel', 1:EEG.nbchan, ...
                                      'Duration', SETTINGS.noActivityDuration, ...
                                      'Flag', 1, ...
                                      'Threshold', [-1*SETTINGS.noActivity/1000, ...
                                                    SETTINGS.noActivity/1000], ...
                                      'Twindow', [SETTINGS.minTimeWindowArtifacts, ...
                                                  SETTINGS.maxTimeWindowArtifacts]); 
     
               EEG = eeg_rejsuperpose( EEG, 1, 1, 1, 1, 1, 1, 1, 1);
               winrej=trial2eegplot(EEG.reject.rejmanual, ...
           ...
           winrej_disp = trial2eegplot(current_rej, ...
                                       current_rejE, ...
                                       EEG_disp.pnts, ...
                                       rejepochcol);
           ...
           saveTmpRej(SETTINGS.filenames{subjectNo}, ...
                      tmprej, ...
                      fullfile(SETTINGS.pathname, 'ArtifactsRejected'))
...
% importChansLocs =========================================================           
        ...
        case 'average'
           ...
           ERP = pop_savemyerp(ERP, ...
                              'erpname', 'ERP', ...
                              'filename', sprintf('%s_ERP.erp', fName), ...
                              'filepath', fullfile(SETTINGS.pathname, 'Averaged_ERP'));
